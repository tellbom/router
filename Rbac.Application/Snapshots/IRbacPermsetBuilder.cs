...
///
/// 核心约束：
/// - 输入只能来自 MySQL 真相库中的用户-组关系、组-权限关系，以及 Casbin policy。
...
{
    /// <summary>
    /// 根据 MySQL/Casbin 派生的 policy 输入构建 permset，并写入 Redis。
    /// </summary>
    /// <param name="input">只接受从 MySQL 真相库派生的策略输入。</param>
    /// <param name="ct"></param>
...
/// <summary>
/// permset 构建输入。只能由 MySQL/Casbin 策略数据填充，禁止来自前端或 ES。
/// </summary>
public sealed class PermsetBuildInput
{
    ...
    /// 此列表只能来自 MySQL 组-权限关系表 + Casbin policy 合并计算。
    /// </summary>
...
{
    /// <summary>来自 MySQL 组-权限关系表 + Casbin policy 计算。（唯一合法来源）</summary>
    MySqlCasbinDerived,

...
internal sealed class AdministratorMapping : IEntityTypeConfiguration<RbacAdministrator>
{
    public void Configure(EntityTypeBuilder<RbacAdministrator> b)
    {
        ...
        // DxEId: 存储为 string，对外永远返回 string
        b.Property(x => x.DxEId)
            .HasColumnName("dxe_id")
            .HasMaxLength(64)
            .HasConversion(v => v.Value, s => new DxEId(s))
            .IsRequired();

        b.Property(x => x.Userid)
            .HasColumnName("userid")
            .HasMaxLength(128)
            .HasConversion(v => v.Value, s => new UserId(s))
            .IsRequired();
...
internal sealed class GroupMapping : IEntityTypeConfiguration<RbacGroup>
{
    public void Configure(EntityTypeBuilder<RbacGroup> b)
    {
        ...
        // RuleCodes / PermissionCodes 存储为 JSON 字符串
        b.Property(x => x.RuleCodes)
            .HasColumnName("rule_codes")
            .HasConversion(
                v => System.Text.Json.JsonSerializer.Serialize(v.Select(r => r.Value).ToList(), (System.Text.Json.JsonSerializerOptions?)null),
                s => (IReadOnlyList<RuleCode>)System.Text.Json.JsonSerializer.Deserialize<List<string>>(s, (System.Text.Json.JsonSerializerOptions?)null)!
                    .Select(r => new RuleCode(r)).ToList());

        b.Property(x => x.PermissionCodes)
            .HasColumnName("permission_codes")
            .HasConversion(
                v => System.Text.Json.JsonSerializer.Serialize(v.Select(p => p.Value).ToList(), (System.Text.Json.JsonSerializerOptions?)null),
                s => (IReadOnlyList<PermissionCode>)System.Text.Json.JsonSerializer.Deserialize<List<string>>(s, (System.Text.Json.JsonSerializerOptions?)null)!
